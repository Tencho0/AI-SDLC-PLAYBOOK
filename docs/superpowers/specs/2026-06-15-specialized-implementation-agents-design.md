...
> **Revision note:** This is v2, revised after an adversarial verification pass (six critics fact-checked v1 against the repo). v2 fixes a cross-layer-integrity blocker, the working-tree concurrency hazard, the inherited characterization-test sequencing, the arg-parsing grammar, the review-owner reconciliation, and the full set of repo files that must stay consistent (README, `playbook/mcp.md`, `playbook/PLAYBOOK.md`, `scripts/verify-scaffold.ps1`).
...
- **Owns only its partitioned files** and **stages but does not commit/push** — the merge pass is the sole committer (see §5b). Shared/cross-cutting files (lockfiles, dependency manifests, DI/route registration, shared type/contract files) are owned by the orchestrator, not a layer agent.
...
**Git discipline:** although layer agents hold `Bash` and `mcp__github__*` (so they can run builds/tests and read repo state), they **must not commit, push, or open PRs** during fan-out — those repo-finalizing operations are reserved for the merge pass. This is stated in each layer agent's body and enforced by convention (see §5b).
...
- **6a. Scope pass** — *only if no layer hint was given:* spawn `implementation` to read the refined story pack + sprint planning pack (+ for inherited, the codebase/architecture map and business-rule recovery report) and the cloned repo, and **return**: (a) the set of participating layers, (b) a per-layer brief (§5a), and (c) a shared cross-layer contract (§5b). If a hint was given, skip the scope pass and use the hinted layers directly. The shared contract (c) is produced only when **two or more** layers participate (a single layer has no cross-layer boundary, so it needs no contract); **a single participating layer — hinted or scope-derived — always takes the fast path (6d) below**, which is the explicit winner whenever the participating-layer count is 1.
- **6b. Fan-out** — dispatch the participating layer agents per the concurrency rules in §5b (parallel for greenfield, sequential safe-order for inherited), passing each its brief, the shared contract, the ticket id, the scenario, and relevant prior artifacts. Each implements + stages its slice and returns its pack-slice.
- **6c. Merge pass** — spawn `implementation` to: integrate the returned slices, **verify code-level cross-layer consistency against the shared contract**, **re-dispatch** any layer agent whose code violates the contract (a bounded remediation loop, default max 1 re-dispatch per layer, then surface to the human), commit the reconciled change as the sole committer, and **write** the completed `<template>` to `src/<eng>/delivery/execution/<ticket>.md`. The merge pass MAY itself make small integrating code edits (e.g. wiring a shared type), but **only within orchestrator-owned shared/cross-cutting files (§5b.2) — never inside a layer's partition**; after any such edit it **re-runs the code-level contract conformance check** before committing.
- **6d. Single-layer fast path** — if exactly one layer participates (hinted or scope-derived), skip the contract/merge passes and spawn that one layer agent directly. **Ordering matters:** the agent first performs layer-scope discovery — if it finds cross-tier work it returns `escalate: needs layers <…>` **before committing or staging anything**, leaving a clean tree, and the command **promotes the ticket to the multi-layer flow** (which then runs under the §5b deferred-commit rules with the merge pass as sole committer). Otherwise it implements, commits, **and writes the pack itself** (orchestrator-lite), filling the template's governance footer and layer-appropriate review note exactly as the merge pass would (preserving guardrails 1/2/7). **For the inherited scenario, the characterization-test gate (§5b.5) runs before this single layer agent mutates code, exactly as in the multi-layer flow** — a lone data migration is the highest-risk case and must not ship without a behavior baseline. (Caveat: a single layer agent sees only its tier's brief, so coverage is *not* literally identical to today's generalist — the escalation path is the safety net.)
...
3. **Deferred commits** — layer agents implement and **stage** but never commit/push. The merge pass is the **sole committer**, so concurrent agents never race on the git index.
4. **Greenfield = parallel** — layer agents run concurrently (safe given 1–3).
5. **Inherited = sequential + characterization gate** — before any layer mutates code, a baseline **characterization-test step locks current behavior**; then layer agents run **sequentially in safe order `data → backend → frontend/mobile`** (migrations are hardest to reverse, so data goes first and behavior is captured before each change). This honors the playbook's "characterization tests before changing behavior" rule, which is unsequenceable under naive parallelism.
...
- **Cross-layer E2E test delegation** to `test-automation` from the orchestrator — deferred; `/qa` remains the owner of deep automated tests. Documented as a seam, not built.
...
3. **Concurrency/integrity:** greenfield multi-layer runs parallel with file-partition + deferred commits + a shared contract; inherited runs sequentially (`data → backend → frontend/mobile`) behind a characterization-test gate that covers **both** single-layer (fast path) and multi-layer tickets; the merge pass verifies code-level contract conformance, confines its own code edits to orchestrator-owned files (re-checking the contract after them), and re-dispatches layer agents on violation. No two agents commit concurrently; fast-path escalation happens before any commit/stage.
...
- **Test boundary:** deep automated tests stay with `/qa` → `test-automation`; layer agents write inline dev tests only.
